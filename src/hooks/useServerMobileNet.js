...
/**
 * Custom hook for MobileNet Server-Side Training
 * Sends data to server for training instead of training in browser
 */
export const useServerMobileNet = () => {
    const [status, setStatus] = useState(`Loading ${getBrandedModelName('MobileNet')}...`);
    ...
    // Simulate model loading (MobileNet is server-side, so just show ready)
    useEffect(() => {
        setTimeout(() => {
            setStatus('Ready');
        }, 500);
    ...
    const addSample = async (classId, imageElement, imageSrc) => {
        // For server-side MobileNet, we just store the image data
        ...
        console.log(`✅ Added sample to class ${classId} for server-side MobileNet`);
    };
    ...
    const trainModel = async (params, userId, projectId) => {
        ...
        try {
            ...
            console.log(`🚀 Sending ${totalSamples} samples to server for MobileNet training...`);
            ...
            const response = await fetch(`${serverUrl}/api/train-mobilenet`, {
                method: 'POST',
                headers: {
    ...
    const predict = async (imageElement, userId, projectId) => {
        // For server-side MobileNet, prediction also happens on server
        if (!userId || !projectId) {
            console.error('Missing userId or projectId for MobileNet prediction');
        ...
        try {
            ...
            const response = await fetch(`${serverUrl}/api/predict-mobilenet`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const exportModel = async (filename) => {
        alert('MobileNet models are stored on the server. Use the API for predictions.');
    };
    ...
    const loadModelFromStorage = async (userId, projectId) => {
        try {
            ...
            if (project && project.model_data && project.model_data.modelType === 'MOBILENET') {
                console.log('✅ Found MobileNet model:', project.model_data);
                    ...
                    const restoredClasses = await Promise.all(
                        project.model_data.classes.map(async (c, index) => {
                            ...
                            if (c.sampleUrls && c.sampleUrls.length > 0) {
                                console.log(`📸 Loading ${c.sampleUrls.length} samples for class '${c.name}'...`);
                                ...
                                const samples = await Promise.all(
                                    c.sampleUrls.map(async (url, idx) => {
                                        try {
                                            ...
                                            console.error(`Failed to load sample ${idx}:`, err);
                                            return null;
                                        }
                                    })
                                );
                                ...
                                console.log(`✅ Loaded ${classData.samples.length} samples for class '${c.name}'`);
                            }
    ...
    const saveModelToStorage = async (userId, projectId) => {
        ...
        console.log('✅ Model already saved to server during training');
        return true;
    };
